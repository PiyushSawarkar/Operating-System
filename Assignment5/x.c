...
shmid = shmget(2220, MAX_STUD_NUM * sizeof(student), IPC_CREAT | 0666); // Shared
memory for student info tables
shmflag = shmget(9990, sizeof(int), IPC_CREAT | 0666); // a bool_changed to see if change
made by y?
/* Checking for errors!!! */
 ...
 *bool_changed = 1;
 ...
 while(counter < count){
 fprintf(fp, "%s %s %d %lf\n", students_info_table[counter].first_name,
students_info_table[counter].last_name, students_info_table[counter].roll_num,
students_info_table[counter].cgpa);
 counter++;
 }
 fclose(fp);
