...
char * getTime()
{
 ...
 strftime(s,sizeof(s),"Msg received at time: %a %b %d %T %Z %Y",
timeptr);
...
msgrcv(UPid, &receive, SIZE_BUF, 0, 0);//server receives a
message from UP queue
 temp1 = time(NULL);
...
result.type = receive.type;//Type is process id of client
process
msgsnd(DOWNid, &result, SIZE_BUF, 0);//Send the message
through DOWN queue.
}
return 0;
}
