...
void fork_pipes (int n, command * cmd)
{
 ...
 int pid, status;
 int in, fd [2];
 /* The first process should get its input from the original file
descriptor 0. */
 if(inFlag)
 {
 ...
 in = 0;
 /* Note the loop bound, we spawn here all, but the last stage of
the pipeline. */
 for (i = 0; i < n - 1; ++i)
 {
 pipe (fd);
 /* f [1] is the write end of the pipe, we carry `in` from
the prev iteration. */
 spawn_proc (in, fd [1], cmd + i);

 /* No need for the write and of the pipe, the child will write
here. */
 close (fd [1]);
 /* Keep the read end of the pipe, the next child will read
from there. */
 in = fd [0];
 }
 /* Last stage of the pipeline - set stdin be the read end of the
previous pipe
 ...
 if(outFlag)
 {
 int ofd = open(outfile, O_CREAT | O_WRONLY, S_IRUSR | S_IWUSR
| S_IRGRP | S_IROTH);
 dup2(ofd, 1);
 }
 openFile(cmd[i].arguments[0], cmd[i].arguments);
 //execvp(cmd [i].arguments [0], (char * const *)cmd
[i].arguments);
}
